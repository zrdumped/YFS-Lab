         ...
         std::string &v)
{
  ...
  std::vector<std::string>::iterator it;
	for (it= nodes.begin(); it < nodes.end(); it++) {
		...
		if (h.safebind()){
			paxos_protocol::status ret = h.safebind()->call(paxos_protocol::preparereq, me, a, r, rpcc::to(1000));
        ...
        std::vector<std::string> nodes, std::string v)
{
  ...
  std::vector<std::string>::iterator it;
	for (it = nodes.begin(); it != nodes.end(); it++) {
		...
		VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
		if (h.safebind()){
				paxos_protocol::status ret = h.safebind()->call(paxos_protocol::acceptreq, me, a, r, rpcc::to(1000));
		  VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
	      ...
	      std::string v)
{
  ...
  std::vector<std::string>::iterator it;
	for (it = accepts.begin(); it != accepts.end(); it++) {
		...
		VERIFY(pthread_mutex_unlock(&pxs_mutex) == 0);
		if (h.safebind())
			paxos_protocol::status ret = h.safebind()->call(paxos_protocol::decidereq, me, a, r, rpcc::to(1000));
		VERIFY(pthread_mutex_lock(&pxs_mutex) == 0);
	}
}
    ...
    paxos_protocol::prepareres &r)
{
  ...
  // Remember to *log* the proposal if the proposal is accepted.
    ...
    // update the number of the highest proposal seen in a prepare
		n_h = a.n;
    //reply prepare_ok(n_a, v_a)
		...
		r.oldinstance = false;
		r.accept = true;
    //*log* the proposal if the proposal is accepted.
		l->logprop(n_h);
	} else {
...
acceptor::acceptreq(std::string src, paxos_protocol::acceptarg a, bool &r)
{
  ...
  // Remember to *log* the accept if the proposal is accepted.
    ...
    //reply accept_ok(n)
		...
		v_a = a.v;
		r = true;
    //*log* the accept if the proposal is accepted.
		l->logaccept(n_a, v_a);
	} else
  //reply accept_reject
		r = false;
