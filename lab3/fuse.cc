...
void
fuseserver_create_symlink(fuse_req_t req, const char* link, fuse_ino_t parent, const char* name){
    int r;
    ...
    e.entry_timeout = 0.0;
    e.generation = 0;
    if ((r = yfs->create_symlink(parent, link, name, ino_out)) == yfs_client::OK) {
        e.ino = ino_out;
...
void
fuseserver_read_symlink(fuse_req_t req, fuse_ino_t ino){
    int r;
    std::string link;
    if((r = yfs->read_symlink(ino, link)) == yfs_client::OK){
...
main(int argc, char *argv[])
{
     ...
     * rmdir, etc.
     * */
    fuseserver_oper.symlink    = fuseserver_create_symlink;
    fuseserver_oper.readlink   = fuseserver_read_symlink;
