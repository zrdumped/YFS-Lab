...
block_manager::block_manager()
{
  ...
  // format the disk
  sb.size = BLOCK_SIZE * BLOCK_NUM / BLOCK_MUL;
  sb.nblocks = BLOCK_NUM / BLOCK_MUL;
...
{
  char code[BLOCK_SIZE * BLOCK_MUL];
  d->read_block(id * BLOCK_MUL, code);
  d->read_block(id * BLOCK_MUL + 1, code + BLOCK_SIZE);
  d->read_block(id * BLOCK_MUL + 2, code + BLOCK_SIZE * 2);
  d->read_block(id * BLOCK_MUL + 3, code + BLOCK_SIZE * 3);
  for(int j = 0; j < 4; j++){
  for(int i = 0; i < BLOCK_SIZE / 4; i++){
    char low, high;
    if(!decode(code[BLOCK_SIZE * j + i * 2], low)){
      if(!decode(code[BLOCK_SIZE * j + BLOCK_SIZE / 2 + i * 2], low)){
        printf("wrong");
      }
    }
    if(!decode(code[BLOCK_SIZE * j + i * 2 + 1], high)){
      if(!decode(code[BLOCK_SIZE * j + BLOCK_SIZE / 2 + i * 2 + 1], high)){
        printf("wrong");
      }
    }
    buf[j * BLOCK_SIZE / 4 + i] = (high << 4) | low;
  }}
  write_block(id, buf);
}

//1byte --hamming code -- 2 byte
//duplicate half of block to the other
//so 2 mistakes per block happen in 1 byte can be solved
void
block_manager::write_block(uint32_t id, const char *buf)
{
  ...
  for(int j = 0; j < 4; j++){
  for(int i = 0; i < BLOCK_SIZE / 4; i++){
    //split 1 byte into 2 and decode them
    char low = encode(buf[j * BLOCK_SIZE / 4 + i] & 0x0F);
    char high = encode((buf[j * BLOCK_SIZE / 4 + i] >> 4) & 0x0F);
      code[BLOCK_SIZE * j + i * 2] = low;
      code[BLOCK_SIZE * j + i * 2 + 1] = high;
      code[BLOCK_SIZE * j + BLOCK_SIZE / 2 + i * 2] = low;
      code[BLOCK_SIZE * j + BLOCK_SIZE / 2 + i * 2 + 1] = high;

  }
}
  d->write_block(id * BLOCK_MUL, code);
  d->write_block(id * BLOCK_MUL + 1, code + BLOCK_SIZE);
  d->write_block(id * BLOCK_MUL + 2, code + BLOCK_SIZE * 2);
  d->write_block(id * BLOCK_MUL + 3, code + BLOCK_SIZE * 3);
}
