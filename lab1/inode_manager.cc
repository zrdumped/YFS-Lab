...
block_manager::alloc_block()
{
   ...
   */
  blockid_t block_num =  BLOCK_SIZE;
  char buf[BLOCK_SIZE];
...
inode_manager::free_inode(uint32_t inum)
{
   ...
   */
   ...
   if(inode_tmp == NULL) {
    printf("\tim: error! inode %d has been free", inum);
    return;
   }
    inode_tmp->type = 0;
...
inode_manager::read_file(uint32_t inum, char **buf_out, int *size)
{
  ...
  inode_t* inode = get_inode(inum);
   if(inode == NULL) {
    printf("\tim: error! inode %d does not exist", inum);
    return;
   }
...
inode_manager::write_file(uint32_t inum, const char *buf, int size)
{
  ...
  bm->read_block(inode->blocks[NDIRECT], tail);
  unsigned int i = 0;
  for(; i < NDIRECT && i < new_blocks_num; i++)
    bm->write_block(inode->blocks[i], buf + BLOCK_SIZE * i);
  //printf("6alloc\n");
  if(i < new_blocks_num){
    for(; i <= NINDIRECT && i < new_blocks_num; i++)
...
inode_manager::remove_file(uint32_t inum)
{
  ...
  if(blocks_num < NDIRECT){
    ...
    char tail[BLOCK_SIZE];
    bm->read_block(inode->blocks[NDIRECT], tail);
    for(unsigned int i = NDIRECT; i < blocks_num; i++){
      bm->free_block(*((blockid_t*)tail + (i - NDIRECT)));
    }
